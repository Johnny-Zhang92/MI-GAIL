...
from mi_model import keras_utils
...
class MiModel(object):
    ...
    """
        ...
        self.nu_net = tf.keras.Sequential([
            tf.keras.layers.Dense(
                hidden_size,
                input_shape=(state_dim + action_dim + state_dim,),
    ...
    """
        ...
        (expert_states, expert_actions,
         expert_next_states, expert_index) = expert_dataset_iter.get_next()
        # expert_target_state = expert_states_instance[expert_index]

        expert_initial_states = expert_states

        # spec = ('observation','action','next_observation','reward','mask',
        # 'target0','target1')
        rb_states, rb_actions, rb_next_states, _, _, \
        rb_target_states0, rb_target_states1 = policy_dataset_iter.get_next()[0]
        ...
        with tf.GradientTape(
            ...
            expert_init_inputs = tf.concat(
                [expert_initial_states, policy_initial_actions, expert_initial_states], 1)

            expert_inputs = tf.concat([expert_states, expert_actions, expert_states], 1)
            expert_next_inputs = tf.concat([expert_next_states,
                                            policy_next_actions,
                                            expert_next_states], 1)

            rb_inputs = tf.concat([rb_states, rb_actions, rb_target_states0], 1)
            rb_next_inputs = tf.concat([rb_next_states, rb_next_actions, rb_target_states1], 1)
