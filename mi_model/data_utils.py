...
def load_expert_data(filename, num_traj):
    """Loads expert trajectoris from a file.

  Args:
    filename: a filename to load the data.
    num_traj: the number of trajectories to load in filename
  ...
  """
    with open(filename, 'rb') as fin:
        ...
        expert_states = expert_data['obs'][:num_traj]
        expert_actions = expert_data['acs'][:num_traj]
        expert_next_states = expert_data['obs1'][:num_traj]
        expert_dones = expert_data['done'][:num_traj]

        expert_states = np.reshape(expert_states, [-1, expert_states.shape[-1]])
        expert_actions = np.reshape(expert_actions, [-1, expert_actions.shape[-1]])
        expert_next_states = np.reshape(expert_next_states, [-1, expert_next_states.shape[-1]])
        expert_dones = np.reshape(expert_dones, [expert_dones.shape[0] * expert_dones.shape[1]])

        rets = expert_data['returns'][:num_traj]  # expert data made by my self
        avg_ret = sum(rets) / len(rets)
        std_ret = np.std(np.array(rets))
        print("Total trajectories: ", num_traj)
        print("Total transitions: ", expert_states.shape[0])
        print("Average returns: ", avg_ret)
        print("Std for returns: ", std_ret)

        expert_index = np.arange(0, expert_states.shape[0])
        expert_index = [int(expert_index[i] % 1000) for i in range(expert_index.shape[0])]
    return expert_states, expert_actions, expert_next_states, expert_dones, expert_index


def subsample_trajectories(expert_states, expert_actions, expert_next_states,
                           expert_dones, num_trajectories, expert_index):
  ...
  """
    ...
    for i in range(expert_states.shape[0]):
        ...
        expert_index_traj[-1].append(expert_index[i])
    ...
    expert_index_traj = [expert_index_traj[i] for i in shuffle_inds]
    ...
    expert_index = concat_trajectories(expert_index_traj)
...
def add_absorbing_states(expert_states, expert_actions, expert_next_states,
  ...
  """
    ...
    expert_index = [x for x in expert_index]
